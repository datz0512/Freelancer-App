...
import { IConversationDocument, IMessageDetails, IMessageDocument, lowerCase } from '@datz0512/freelancer-shared';
...
const addMessage = async (data: IMessageDocument): Promise<IMessageDocument> => {
  ...
  socketIOChatObject.emit('message received', message); //sent to API gateway
  return message;
};
...
const markMessageAsRead = async (messageId: string): Promise<IMessageDocument> => {
  ...
  socketIOChatObject.emit('message updated', message); //sent to API gateway
  return message;
};

const markManyMessageAsRead = async (
  sender: string,
  receiver: string,
  messageId: string
  ...
  const message: IMessageDocument = (await MessageModel.findById(messageId)) as IMessageDocument;
  socketIOChatObject.emit('message updated', message); //sent to API gateway
  return message;
};
