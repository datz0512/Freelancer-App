...
describe('Email Consumer', () => {
  ...
  describe('consumeAuthEmailMessages method', () => {
    it('should be called', async () => {
      ...
      expect(connectionChannel!.assertExchange).toHaveBeenCalledWith('freelancer-email-notification', 'direct');
      expect(connectionChannel!.assertQueue).toHaveBeenCalledTimes(1);
      expect(connectionChannel!.consume).toHaveBeenCalledTimes(1);
      expect(connectionChannel!.bindQueue).toHaveBeenCalledWith('auth-email-queue', 'freelancer-email-notification', 'auth-email');
    });
  });
  ...
  describe('consumeOrderEmailMessages method', () => {
    it('should be called', async () => {
      ...
      expect(connectionChannel!.assertExchange).toHaveBeenCalledWith('freelancer-order-notification', 'direct');
      expect(connectionChannel!.assertQueue).toHaveBeenCalledTimes(1);
      expect(connectionChannel!.consume).toHaveBeenCalledTimes(1);
      expect(connectionChannel!.bindQueue).toHaveBeenCalledWith('order-email-queue', 'freelancer-order-notification', 'order-email');
    });
  });
});
